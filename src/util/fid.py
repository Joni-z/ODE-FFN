...
def get_activations(
    ...
    # for irregular sized dataset (e.g. ImageNet with varying resolutions)
...
class ImageTensorDataset(torch.utils.data.Dataset):
    def __init__(self, tensor):
        self.tensor = tensor # Assuming between 0 and 1
...
def compute_statistics_of_tensor(
    tensor, model, batch_size=50, dims=2048, device="cuda", num_workers=4
):
    """Calculates the activations of the pool_3 layer for all images.
    ...
    dataset = ImageTensorDataset(tensor)
    dataloader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
...
def calculate_fid(tensor, batch_size=50, device='cuda', dims=2048, num_workers=4,
                  ref_path="/vast/hl3797/mlp-diffusion/util/ffhq.npz"):
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx]).to(device)
    assert ref_path.endswith(".npz"), "Reference path must be a .npz file"
    with np.load(ref_path) as f:
        m1, s1 = f["mu"][:], f["sigma"][:]
    m2, s2 = compute_statistics_of_tensor(tensor, model, batch_size, dims, device, num_workers)
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)

    return fid_value


def calculate_fid_path(path, batch_size=100, device='cuda', dims=2048, num_workers=4,
                  ref_path="/vast/hl3797/mlp-diffusion/util/ffhq.npz"):
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx]).to(device)
    assert ref_path.endswith(".npz"), "Reference path must be a .npz file"
